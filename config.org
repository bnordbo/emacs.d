...
(use-package consult
  ...
  (("M-y" . consult-yank-pop)          ; like yank pop, but also shows preview
   ("C-c s f" . consult-find           ; (f)ind file
   ("C-c s g" . consult-ripgrep)       ; (g)rep files
   ("C-c s s" . consult-line)          ; (s)earch for line
   ("C-c s i" . consult-imenu)         ; (i)menu completions
   ("C-c s j" . consult-mark)          ; (j)ump to mark
   ("C-c s f" . consult-focus-lines))  ; (f)ilter lines
  :init
...
#+begin_src emacs-lisp
; TODO: Find a replacement for helm-rg
#+end_src
...
Set up org-roam v2 using quelpa and use-package, and bind it behind the ~C-c n~ prefix:
...
Forge hooks into magit and provides access to GitHub. It's quite slow in the
...
This allows visiting the current file in the remote repo. Very useful for
giving others references to code.
