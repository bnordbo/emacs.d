...
Load my local functions and keyboard macros.

#+begin_src emacs-lisp
(load-file "~/.config/emacs/elisp/bn.el")
(load-file "~/.config/emacs/keyboard-macros.el")
#+end_src
...
Prevent customise from polluting the init files.
...
#+begin_src emacs-lisp
(load-file "~/.config/emacs/elisp/sensible-defaults.el")
...
After switching to native compilation, the warning buffer started to pop up a lot. Increasing the log level to ~error~ takes care of that annoyance.

#+begin_src emacs-lisp
(setq warning-minimum-level :error)
#+end_src

Allow using the system trash can instead of deleting files.
...
This adds a lot of nifty utilities that I've been missing.
...
(defvar theme-sets
  '((leuven ((light . leuven) (dark . leuven-dark)))
    ...
    (poet ((light . poet) (dark . poet-dark)))
    (material ((light . material-light) (dark . material)))))

(setq default-theme 'gruvbox-light-medium)
#+end_src

Adjust the theme according to the system mode.
...
#+begin_src emacs-lisp
(when "Fira Code" (font-family-list)
  (set-face-attribute 'default nil :font "Fira Code" :height 130)
  (set-face-attribute 'fixed-pitch nil :font "Fira Code"))
(when "Libre Baskerville" (font-family-list)
  (set-face-attribute 'variable-pitch nil :font "Libre Baskerville" :height 140))
#+end_src

This doesn't work as prettify-symbols-alist becomes buffer local. It may also not be ideal to do this unconditionally for or buffers. I should set this per mode instead.
...
I can't see why Emacs should not have ultrafine scrolling just like everything else.
...
The free-keys package prints available key bindings. This is invaluable on the rare occasion that it's needed.

#+begin_src emacs-lisp
(use-package free-keys)
#+end_src

These are my own key maps.
...
Rebind ~C-x r b~ to use Consult for bookmarking.
...
** Casual
Transient is nice, so use it for org-agenda and bookmarks as well.

#+begin_src emacs-lisp
(use-package casual
  :bind (:map org-agenda-keymap ("C-o" . casual-agenda-tmenu)
...
I just use ispell with Hunspell for now, but should look into a way of supporting multiple dictionaries.
...
This allows undoing keyboard macro invocations atomically.
...
(use-package projectile
  ...
  :init (setq projectile-project-search-path '(("~/Contexts/WG2/src" . 2)
                                               "/Users/bn/.config/emacs"
                                               "~/src/forks/qmk_firmware"))
  :bind-keymap ("C-c p" . projectile-command-map)
  :config (projectile-mode +1))
#+end_src

Disproject looks like an interesting "alternative" to projectile:
...
(use-package svelte-ts-mode
  :load-path "/Users/bn/.local/src/svelte-ts-mode")
#+end_src
...
(use-package org
  ...
  (setq org-startup-folded 'show2levels
        ...
        org-ditaa-jar-path "/Users/bn/.config/emacs/ditaa.jar"
        org-cycle-emulate-tab 'whitestart
        ; Improve org-hoto by allowing completion on the full outline
...
- TODO for incomplete tasks. If used together with NEXT, it means they are not yet ready.
- NEXT for tasks that are ready to start.
...
This replaces ~org-superstar~ and adds lots of niceties.

#+begin_src emacs-lisp
(use-package org-modern
  :config
  (set-face-attribute 'org-modern-symbol nil :family "Libre Baskerville")
  (global-org-modern-mode))
#+end_src

Also use ~org-modern-indent~ form a local clone. I should start using straight...

#+begin_src emacs-lisp
(load-file "~/.local/src/org-modern-indent/org-modern-indent.el")
#+end_src
...
Override the variable pitch font as appropriate.
...
Use olivetti-mode to make the content centered and not overly wide.
...
#+begin_src emacs-lisp
(customize-set-value 'org-auto-align-tags t)
...
** Extras
Support for pasting and dropping images. Setting ~org-image-actual-width~ to ~nil~ allows specifying e.g. ~#+org_attr: :width 600~ to limit the width of the image, otherwise they sometimes appear too wide.
...
Mode specific scratch buffers are not specific to org-mode, but often used in conjunction with it for quick notes.

#+begin_src emacs-lisp
(use-package scratch)
#+end_src

It's sometimes convenient to place tasks inside the text body. By ~org-inlinetask~ adds support for this, bound to ~C-c C-x t~.
...
** Agenda setup
I haven't used org-agenda for a while. Back then I had a plan (that maybe sorta worked). Now I need to adapt it to whatever I have.

#+begin_src emacs-lisp
(setq org-agenda-files
      '("~/Repository/Org/Employment.org"
        "~/Repository/Org/CiscoSubnators.org"
        "~/Repository/Org/Diary.org"
        "~/Repository/Org/Progress.org"))
#+end_src

Custom commands: Listnig the agenda along with TODOs is good for overview, and just the daily agenda for focus.
...
Configure the agenda window setup. I prefer that it doesn't change it a lot.
...
(setq org-capture-templates
      `(("t" "TODO" entry (file+headline org-default-notes-file "Incoming")
        ...
        ("s" "Standup" plain
         (file+olp+datetree "WG2SubscriberState.org" "Standups")
         (file "~/.config/emacs/templates/standup-capture.org")
         :immediate-finish t
         :jump-to-captured t)
        ("C" "Contact" entry (file "Contacts.org")
         (file "~/.config/emacs/templates/contact-capture.org"))
        ("t" "Training")
	("tw" "Weekly status" entry
         (file+olp+datetree "Training.org" "Ukesstatus")
         (file "~/.config/emacs/templates/training-weekly-capture.org")
         :tree-type week)))
#+end_src
...
(use-package org-roam
  ...
  (org-roam-capture-templates '(("d" "Default" plain "%?"
                                 :target (file+head "%<%Y%m%d%H%M%S>-${slug}.org"
                                                    "#+date: %T\n#+title: ${title}\n")
                                 :unnarrowed t)
                                ("r" "Read" plain (file "~/.config/emacs/templates/book-capture.org")
                                 :target (file+head "%<%Y%m%d%H%M%S>-${slug}.org"
                                                    "#+title: ${title}\n#+date %T\n#+filetags: Lit\n")
                                 :unnarrowed t)
                                ("p" "Person" plain (file "~/.config/emacs/templates/person-capture.org")
                                 :target (file+head "%<%Y%m%d%H%M%S>-${slug}.org"
                                                    "#+title: ${title}\n#+date %T\n#+filetags: Person\n")
                                 :unnarrowed t)))
  :config
...
(use-package consult-org-roam
  :load-path "~/.local/src/consult-org-roam"
  :ensure t
  :after org-roam
  :init
...
* Treesitter experiment
Taken from [[https://www.ovistoica.com/blog/2024-7-05-modern-emacs-typescript-web-tsx-config#orgf6d33f7][this blog post]].
#+begin_src emacs-lisp
(use-package tree-sitter
  ...
  (use-package combobulate
    ...
    ;; code.
    :load-path ("~/.local/src/combom/combobulate")))
